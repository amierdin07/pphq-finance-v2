...
const ExportModal: React.FC<ExportModalProps> = ({ isOpen, onClose, transactions, branches, title, branchName, mode = 'detailed' }) => {
    ...
    const handleDownloadPDF = () => {
        ...
        if (settings.appLogoUrl) {
            try {
                ...
                doc.text('E-STATEMENT PPHQ FINANCE', 38, 13);
                ...
                doc.text(mode === 'summary' ? 'Summary Performance Per Cabang' : 'Laporan Pemasukan & Pengeluaran Kas', 38, 26);
            } catch (e) {
                // Fallback if image fails
                doc.setFont('helvetica', 'bold');
                doc.setFontSize(18);
                doc.setTextColor(255, 255, 255);
                doc.text('E-STATEMENT PPHQ FINANCE', pageW / 2, 13, { align: 'center' });
            ...
            doc.setFontSize(8);
            doc.text(mode === 'summary' ? 'Summary Performance Per Cabang' : 'Laporan Pemasukan & Pengeluaran Kas', pageW / 2, 27, { align: 'center' });
        }
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(9);
        const periodText = `Periode: ${new Date(startDate).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' })} – ${new Date(endDate).toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' })}`;
        ...
        const filename = `${mode === 'summary' ? 'Summary' : 'E-Statement'}_${displayBranch.replace(/\s/g, '_')}_${startDate}_sd_${endDate}.pdf`;
        doc.save(filename);
        onClose();
    };
    ...
    const handlePrint = () => {
        ...
        <div class="header-bar">
            ...
            <div class="header-text">
                <h1>E-STATEMENT PPHQ FINANCE</h1>
                <div class="branch-name">${mode === 'summary' ? 'LAPORAN RINGKASAN UNIT' : displayBranch.toUpperCase()}</div>
                <div class="sub-title">${mode === 'summary' ? 'Summary Performance Per Cabang' : 'Laporan Pemasukan & Pengeluaran Kas'}</div>
            </div>
        </div>
        <div class="meta">
            <span><strong>PERIODE:</strong> ${new Date(startDate).toLocaleDateString('id-ID',{day:'2-digit',month:'long',year:'numeric'})} – ${new Date(endDate).toLocaleDateString('id-ID',{day:'2-digit',month:'long',year:'numeric'})}</span>
            <span><strong>DICETAK:</strong> ${new Date().toLocaleString('id-ID')}</span>
        </div>
        <table><thead><tr><th>No</th>${mode === 'summary' ? '<th>Cabang</th><th>Pemasukan</th><th>Pengeluaran</th><th style="text-align:right">Saldo Bersih</th>' : '<th>Tanggal</th><th>Deskripsi</th><th>Kategori</th><th>Tipe</th><th style="text-align:right">Jumlah</th>'}</tr></thead>
        <tbody>${mode === 'summary' ? getBranchSummary(filtered).map((s,i)=>`<tr><td>${i+1}</td><td>${s.name}</td><td class="in">Rp${s.income.toLocaleString('id-ID')}</td><td class="out">Rp${s.expense.toLocaleString('id-ID')}</td><td style="text-align:right;font-weight:700">Rp${s.balance.toLocaleString('id-ID')}</td></tr>`).join('') : filtered.map((t,i)=>`<tr><td>${i+1}</td><td>${new Date(t.date).toLocaleDateString('id-ID')}</td><td>${t.description}</td><td>${t.category}</td><td class="${t.type===TransactionType.Income?'in':'out'}">${t.type===TransactionType.Income?'Masuk':'Keluar'}</td><td style="text-align:right" class="${t.type===TransactionType.Income?'in':'out'}">${t.type===TransactionType.Income?'+':'-'}Rp${t.amount.toLocaleString('id-ID')}</td></tr>`).join('')}</tbody></table>
    ...
    const handleExportCSV = () => {
        ...
        link.download = `${mode === 'summary' ? 'Summary' : 'E-Statement'}_${displayBranch.replace(/\s/g, '_')}_${startDate}_sd_${endDate}.csv`;
        link.click();
        onClose();
    };
